...
class SimpleTradingBot:
    ...
    def _perform_technical_analysis(self, symbol):
        """执行技术分析 - 修复版"""
        try:
            # 获取市场数据
            df = self.get_market_data_sync(symbol)
            if df.empty:
                self.logger.warning(f"{symbol} 没有足够的数据进行技术分析")
            ...
            result = {
                'symbol': symbol,
                'current_price': float(df['close'].iloc[-1]) if 'close' in df else 0
            }
            ...
            if all(col in df.columns for col in ['MACD', 'MACD_signal']):
                ...
                result['macd'] = float(macd) if not pd.isna(macd) else 0
                result['macd_signal'] = float(macd_signal) if not pd.isna(macd_signal) else 0
                result['macd_histogram'] = result['macd'] - result['macd_signal']

                # MACD信号判断
                if result['macd'] > result['macd_signal']:
                    result['momentum'] = {'macd_signal': 'BULLISH'}
                else:
                    result['momentum'] = {'macd_signal': 'BEARISH'}
            ...
            if all(col in df.columns for col in ['BB_Upper', 'BB_Lower', 'BB_Middle']):
                ...
                if bb_upper > bb_lower:
                    result['bb_position'] = ((current_price - bb_lower) / (bb_upper - bb_lower)) * 100
                else:
            ...
            if 'Williams_R' in df.columns:
                ...
                if len(williams_r_values) > 0:
                    result['williams_r'] = float(williams_r_values.iloc[-1])
                else:
            ...
            if 'CCI' in df.columns:
                ...
                if len(cci_values) > 0:
                    result['cci'] = float(cci_values.iloc[-1])
                else:
                    result['cci'] = 0
            ...
            if 'ADX' in df.columns:
                ...
                if len(adx_values) > 0:
                    adx_value = float(adx_values.iloc[-1])
                    result['ADX'] = adx_value
                    result['adx'] = adx_value  # 添加小写版本！
                    print(f"  ADX 设置为: {adx_value}")
                else:
                    result['ADX'] = 20
                    result['adx'] = 20
            else:
            ...
            if 'volume' in df.columns and len(df) >= 2:
                ...
                if prev_vol > 0:
                    result['volume_ratio'] = current_vol / prev_vol
                else:
                    result['volume_ratio'] = 1.0

                result['volume'] = {'ratio': result['volume_ratio']}
            ...
            if 'ATR' in df.columns:
                ...
                if len(atr_values) > 0:
                    ...
                    result['atr'] = atr_current  # 添加小写版本

                    # ATR 比率
                    if len(df) >= 20:
                        atr_mean = df['ATR'].rolling(20).mean()
                        if len(atr_mean.dropna()) > 0 and atr_mean.iloc[-1] > 0:
                            atr_ratio = atr_current / float(atr_mean.iloc[-1])
                            result['ATR_ratio'] = atr_ratio
                            result['atr_ratio'] = atr_ratio  # 添加小写版本
                        else:
            ...
            # 打印趋势信息
            print(f"  ✓ 趋势分析: {trend_info['direction']} (强度: {trend_info['strength']:.2f})")
            ...
            # MACD信号
            if result.get('momentum', {}).get('macd_signal') == 'BULLISH':
                signal_strength += 1
            else:
                signal_strength -= 1
            ...
            # 趋势信号
            if trend_info['direction'] == 'UP':
                signal_strength += trend_info['strength'] * 2
            ...
            print(f"  ✓ ADX: {result.get('adx', 20):.1f}")
            print(f"  ✓ 布林带位置: {result.get('bb_position', 50):.1f}%")
            print(f"  ✓ 成交量比率: {result.get('volume_ratio', 1.0):.2f}x")
            print(f"  ✓ 趋势: {result.get('trend_direction', 'NEUTRAL')} (强度: {trend_info['strength']:.2f})")

            return result

        except Exception as e:
            self.logger.error(f"{symbol} 技术分析错误: {e}")
            import traceback
            traceback.print_exc()
            return self.get_default_technical_analysis(symbol)
